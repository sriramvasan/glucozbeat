...
const WhyDoesThisMatter = ({ children }: { children: ReactNode }) => {
  return (
    <div className="collapse mt-1">
      ...
      <div className="inline-flex items-center collapse-title p-0 min-h-0 text-gray-500">
        <TbInfoCircle className="pr-1 size-5" />
  ...
  return (
    <>
      ...
      <p className="regular-20">
        <span className="bold-purple">Gestational diabetes</span> is a type of
        diabetes that can happen during pregnancy. Most women are checked for
        gestational diabetes{" "}
        <span className="bold-purple">
          between 24 and 28 weeks of pregnancy
        </span>
        . Since this condition usually doesn&apos;t cause any symptoms, testing
        is very important to keep you and your baby healthy.
      </p>
      <p className="regular-20">
      ...
      <div className="mt-1 text-gray-500">
        <p className="text-sm">
          This Gestational health check was developed by EZPZ based on
          Australian Institute of Health and Welfare 2021 Diabetes statistics in
          combination with other academic studies. This is not intended to be a
  ...
  return (
    <>
      <div>
        ...
        <div className="px-8 flex flex-col lg:flex-row items-center justify-center mt-4 [&_input.grow]:max-w-32">
          ...
          <label className="form-control relative">
            <div className="input input-bordered flex items-center gap-2">
              BMI
              <input readOnly value={bmi} className="grow" />
            </div>
            <div className="label absolute top-10">
              <span className="label-text-alt text-error">
                {Number(bmi) > 100 && "BMI is too high"}
              </span>
            </div>
          </label>
        </div>
      ...
      <StepNavigation
        ...
        nextStep={nextStep}
        eligibleToProceed={
          isFormDataValid("weight") && isFormDataValid("height") && Number(bmi) <= 100
        }
      />
    </>
  );
