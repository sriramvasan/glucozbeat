...
const RecipeFinder = () => {
  ...
  const searchIngredients = async (searchTerm: string) => {
    if (searchTerm.length > 0){
      try{
        ...
        setIngredients(data.map((item: any) => item.Foods));
      }
      catch(error){
  ...
  const handleGenerateRecipes = async () => {
    // console.log(selectedIngredients, mustHave)
    setIsLoading(true);

    try {
      // Fetch and parse the response as JSON
      const response = await generateRecipes(cuisine, selectedIngredients.join(" "), mustHave);
      ...
      setRecipes([]);  // Ensure recipes is set to an empty array if there is an error
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="flex flex-col items-center p-6 bg-gray-100">
      ...
      <div className="flex w-full max-w-4xl bg-white shadow-lg rounded-lg p-6">
        ...
        <div className="flex-1 border-r border-gray-200 pr-6">
          ...
          <p className="text-gray-600 mb-4">
            Start typing the ingredients, and choose from the list of ingredients you can choose for your daily cooking
          </p>
          <input
            type="text"
          ...
          <select
              ...
              >
                <option defaultValue="">Select a Cuisine</option>
        ...
        <div className="flex-1 pl-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <button
              onClick={() => {setSelectedIngredients([]); setCuisine(''); }}
          ...
          {selectedIngredients.length > 2 && (
            <div>
              ...
              >
                <option defaultValue="">Select an ingredient</option>
